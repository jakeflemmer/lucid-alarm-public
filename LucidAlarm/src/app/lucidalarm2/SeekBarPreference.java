...
public class SeekBarPreference extends Preference implements OnSeekBarChangeListener {
	...
	private static final String APPLICATIONNS="http://lucidalarm.com";
	...
	private void initPreference(Context context, AttributeSet attrs) {
		...
		setWidgetLayoutResource(R.layout.seek_bar_preference_layout);
	}
	...
	protected void updateView(View view) {
		...
		try {
			...
			mStatusText.setText(mCurrentValueString);
			mStatusText.setMinimumWidth(30);
			
			mSeekBar.setProgress( Integer.parseInt(mCurrentValueString) - mMinValue);
	...
	public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
		...
		if(!callChangeListener(newValue)){
			seekBar.setProgress( Integer.parseInt(mCurrentValueString) - mMinValue); 
			return; 
		}

		// change accepted, store it
		mCurrentValueString = Integer.toString( newValue );
		mStatusText.setText(String.valueOf(newValue));
		persistString(mCurrentValueString);
	...
	protected void onSetInitialValue(boolean restoreValue, Object defaultValue) {
		...
		if(restoreValue) {
			mCurrentValueString = getPersistedString( mCurrentValueString );
		}
		else {
			int temp = 0;
			try {
			...
			mCurrentValueString = Integer.toString( temp );
		}
